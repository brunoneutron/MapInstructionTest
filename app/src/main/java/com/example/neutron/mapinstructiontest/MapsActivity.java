package com.example.neutron.mapinstructiontest;
...
import com.example.neutron.mapinstructiontest.MAPBOX.MAPBOXAPI;
import com.example.neutron.mapinstructiontest.OSRMAPI.Geometry;
import com.example.neutron.mapinstructiontest.OSRMAPI.Legs;
import com.example.neutron.mapinstructiontest.OSRMAPI.Maneuver;
import com.example.neutron.mapinstructiontest.OSRMAPI.Matchings;
import com.example.neutron.mapinstructiontest.OSRMAPI.OSRMAPIMATCH;
import com.example.neutron.mapinstructiontest.OSRMAPI.Steps;
import com.example.neutron.mapinstructiontest.OSRMROUTEAPI.OSRMROUTE;
import com.example.neutron.mapinstructiontest.OSRMROUTEAPI.Routes;
...
public class MapsActivity extends AppCompatActivity implements
    ...
    boolean instrThreadworking = false;

    //Boolean to check if this is the first instruction we are receiving
    private boolean isFirstStart = true;

    private GoogleApiClient client2;

    private static final String short_url = "http://10.20.24.197:5000/match/v1/driving/129.187424,35.576461;129.189622,35.575741;129.191489,35.574004?timestamps=1412229071;1412229091;1412229191&steps=true&geometries=geojson&overview=full";
    private static final String BASE_URL_RoundAbout = "http://10.20.24.197:5000/match/v1/driving/129.191360,35.572605;129.191513,35.572904;129.192114,35.572941;129.192734,35.572830;129.193346,35.572662;129.193874,35.572481;129.194499,35.572219;129.194834,35.572004;129.195169,35.572006;129.195011,35.572261;129.194370,35.572501;129.193783,35.572724;129.193067,35.572940;129.192450,35.573099;129.192160,35.573494;129.191648,35.573732?timestamps=1412229071;1412229091;1412229191;1412229291;1412229391;1412229491;1412229591;1412229691;1412229791;1412229891;1412229991;1412230091;1412230191;1412230291;1412230391;1412230491&steps=true&geometries=geojson&overview=full";

    private static final String LongerRoute = "http://10.20.24.197:5000/match/v1/driving/129.187410,35.576472;129.188353,35.576188;129.188889,35.575739;129.189565,35.575730;129.190409,35.575534;129.191110,35.574683;129.191502,35.573981;129.191609,35.573401;129.191802,35.572904;129.192864,35.572786;129.194151,35.572376;129.195074,35.571957;129.195905,35.571948;129.197557,35.571983;129.199107,35.572171;129.200427,35.572302;129.199826,35.572407?timestamps=1412229071;1412229076;1412229081;1412229086;1412229091;1412229096;1412229101;1412229106;1412229111;1412229116;1412229121;1412229126;1412229131;1412229136;1412229141;1412229146;1412229151;1412229156;1412229161;1412229166;1412229171&steps=true&geometries=geojson&overview=full";
    ...
    List<LatLng> instrucloc_clean = new ArrayList<>();
    List<String> instruction_clean = new ArrayList<>();

    int index = 0;
    Marker curr_marker = null;

    boolean start_Journey = true;
    ...
    public void onLocationChanged(Location location) {
        ...
        updateMarkers();
        //we must show instruction only when instructions are available
        show_instruction();
    ...
    private void show_instruction() {
        /**
         * First we want to implement a simple navigation system.
         * Just show the next instruction based on the distance between
         * our current location and the shortest instruction.
         * However we want to display the instructions in order.
         */
        double distance_ = 10000.0;
        Location mylocation = mCurrentLocation;
        LatLng shortest_distance = new LatLng(0.0,0.0);


        if(!instrThreadworking){
            instrThreadworking = true;

            if(instrucloc_clean.isEmpty()){
                Toast toast = Toast.makeText(getApplicationContext(),"No instructions availabale",Toast.LENGTH_LONG);
                toast.show();

            }else{
                /**
                 * Since cleanup() has already been called, our instruction starts from the first
                 */
                shortest_distance = instrucloc_clean.get(index);
                Location loc = new Location("Location of instruction");
                loc.setLatitude(shortest_distance.latitude);
                loc.setLongitude(shortest_distance.longitude);
                distance_ = mylocation.distanceTo(loc);
                //Now we get the instruction we have to show
                String instruction_ =  instruction_clean.get(index);
                Double double_instance = new Double(distance_);
                Toast toast = Toast.makeText(getApplicationContext(),double_instance.toString()+"\n"+instruction_,Toast.LENGTH_LONG);
                toast.show();
                if(shortest_distance.latitude != 0.0 && shortest_distance.longitude != 0.0 && distance_ >= 40.0) {
                    if (curr_marker == null) {
                        curr_marker = mMap.addMarker(new MarkerOptions()
                                .position(shortest_distance)
...
/*
    ...
    public void getOSRMData(){
        /**
         * Bypassing OSRM server query for rapid testing
         */
        MatchRequestClient(test_reponse);
    ...
    private void cleanup(){
        /**
         * (-) means not inplemented yet
         * In this method, we take instruction location container and do the following:
         *  1. First set isFirstStart to false. -->Thus we just got our first instruction set
         *  2. Find the LatLng coordinate that gives the shortest distance between our current
         *      location and any of the instructions. Thus we are closer to this instruction that
         *      any other instruction.
         *  3. If this LatLng is the first element in our instruction container, we do nothing since this means
         *      we are starting from the expected start position. Go ahead and show the instructions as it is.
         *      (just exit this method)
         *  4. If this LatLng is not the first instruction, then we have to remove all LatLng coordinates before it
         *      since we wont show them.
         *  -5. If the LatLng is the last one, then we are already at our destination and have to do nothing
         */
        //1.
        int index_loc = -1;
        isFirstStart = false;
        ...
        //we need to check if we have any instructions at all. If none, do nothing.
        if(instruc_loc.isEmpty()){
            Toast toast = Toast.makeText(getApplicationContext(),"No instructions available", Toast.LENGTH_LONG);
            toast.show();
        }else{
            //Now we look for the shortest distance. Iterate through instruc_loc
            for(int i=0;i<instruc_loc.size();i++){
                LatLng latLng = instruc_loc.get(i);
                //create a new location variable to hold our current latlng
                Location loc = new Location("Current Location");
                loc.setLatitude(latLng.latitude);
                loc.setLongitude(latLng.longitude);
                //Here we find the distance between mylocation and loc
                double distance_ = mylocation.distanceTo(loc);
                //check if distance_ is smaller than the base set value of distance(above)
                if(distance_ <= distance){
                    distance = distance_;
                    //Store the index. That is what we need for the cleanup
                    index_loc = i;
                }
            }
            //3,4  somehow remove is not working so we are going to create a new list for valid commands
            /*longInfo(String.valueOf(index));
            if(index != -1){
                for(int i=0;i<index;i++){
                    instruc_loc.remove(i);
                    //We also have to remove the instruction strings
                    instruction.remove(i);
                }
            }*/
            /**
             * TODO: This piece of code can be optimized.
             * If we start from the begining(first instruction location) then
             * there's no need to do the recopying below. For now this is left
             * for testing purposes. But IT MUST BE FIXED!!!(Just check index_loc
             *  if it's 0 then we dont need to do the recopying)
             */
            if(index_loc != -1){
                for(int i=index_loc;i<instruc_loc.size();i++){
                    instrucloc_clean.add(instruc_loc.get(i));
                    instruction_clean.add(instruction.get(i));
                }
            }

            //free the space occupied by instruc_loc and instruction
            instruc_loc.clear();
            instruction.clear();
        }
    ...
    private void  MatchRequestClient(String response){
        ...
        /**
         * After getting all the instruction and the latitiude longitude pairs,
         * we call a function that cleans it up(Remove some instructions that are
         * behind us. This is the cleanup function. We call this function when we
         * have a new instruction. Thus when isFirstStart == true
         */
        //longInfo(String.valueOf(instruc_loc.size()));
        if(!instruc_loc.isEmpty()) {
                ...
                public void run() {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            for (int i = 0; i < instrucloc_clean.size(); i++) {
                                String instruc = instruction_clean.get(i);
                                LatLng latLng = instrucloc_clean.get(i);
